...
def train() -> None:
    ...
    for epoch in range(10):
        ...
        for i, (img, target) in enumerate(train_dataloader):
            ...
            wandb.log({"gradients": wandb.Histogram(model.conv1.weight.grad.cpu().detach().numpy())})
    ...
    wandb.log({"sample_image": wandb.Image(sample_img[0].cpu())})

    # Log a histogram of the model's weights
    wandb.log({"conv1_weights": wandb.Histogram(model.conv1.weight.cpu().detach().numpy())})
